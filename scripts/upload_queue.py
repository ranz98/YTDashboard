...
def scan():
    ...
    for folder in VIDEO_DIR.rglob("*"):
        ...
        # Two things write this file. This script keys records by video id;
        # trainer.py keys them by the staged filename, because the name it
        # uploads is the rewritten title and carries no id. Check both, or a
        # video uploaded by trainer.py still shows up here as "ready".
        record = state.get(video_id)
        if record is None and final is not None:
            record = state.get(final.name)
...
def main():
    ...
    for r in rows:
        ...
        if r["record"]:
            # Records written by trainer.py carry only a path and timestamp;
            # ones written here also have a YouTube URL and privacy setting.
            url = r["record"].get("url")
            if url:
                print(f"     {url}  ({r['record'].get('privacy', '?')})")
            else:
                print(f"     uploaded {r['record'].get('uploaded_at', '?')}")
    print("  " + "-" * 74)
