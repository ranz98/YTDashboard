...
BUTTON_DIR = BASE / "assets" / "buttons"
LOG_DIR = BASE / "output" / "logs"
DEBUG_DIR = LOG_DIR / "debug"

# Where finished videos land after cover_caption.py stages them for upload.
# UPLOAD_VIDEO_SOURCE (env var) is always tried first; then this folder; then
# the fallback below.
DEFAULT_VIDEO_SOURCES = [
    str(BASE / "output" / "videos" / "toupload"),
    r"D:\testvideos",
]

# Records which files have been uploaded. Once a filename is in here, the auto
# source picker skips it — so a re-run with no new video no longer picks up the
# last upload. Explicit "python trainer.py <path\to\file.mp4>" still bypasses
# the filter, so it can force a re-upload when the operator really means it.
UPLOAD_STATE_FILE = BASE / "output" / "uploaded.json"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Report which video would be uploaded and exit without clicking "
            "anything. Exit 0 = one is ready, 2 = nothing new to upload. Lets "
            "uploader.bat decide whether to open Chrome at all."
        ),
    )
...
def _load_upload_state():
    """Read the persistent record of files already uploaded.

    Missing or malformed file is treated as an empty record so a fresh run just
    starts tracking from scratch. Never crash the uploader over its own state
    file - the worst case is a duplicate upload, and the file gets rewritten
    on the next success either way.
    """
    if not UPLOAD_STATE_FILE.exists():
        return {}
    try:
        data = json.loads(UPLOAD_STATE_FILE.read_text(encoding="utf-8"))
    except (ValueError, OSError):
        return {}
    return data if isinstance(data, dict) else {}


def already_uploaded_filenames():
    """The set of basenames the record says have already been uploaded."""
    state = _load_upload_state()
    names = set()
    for key, record in state.items():
        # Records written by this script are keyed by the file's basename.
        # upload_queue.py may also write video-id records here; those keys
        # contain no path separators and no extension, so they never match a
        # real filename we would find in the toupload folder.
...
def mark_upload_recorded(video_path):
    """Write down that this file has been uploaded, so the next auto-pick skips it.

    Called right after click_publish returns, before the file is moved out of
    the source folder. The move is best-effort tidy-up; this record is what
    actually protects against re-uploading, and it survives the file being
    moved, renamed, or the toupload folder being restored from a backup.
    """
    state = _load_upload_state()
    state[video_path.name] = {
        "path": str(video_path),
...
def newest_video_in(folder, skip_names=None):
    """Newest video in `folder`, skipping any basename in `skip_names`.

    Called from the auto source-picker only. An explicit file path passed on
    the command line bypasses this and its filter entirely, so the operator
    can still force a re-upload of a specific file when they mean it.
    """
...
def choose_video(source):
    """Pick the next video to upload.

    Returns a Path when there is something to upload, or None (auto mode only)
    when every candidate folder is exhausted because every video in it is
    already recorded in `uploaded.json`. The caller treats None as a clean
    "nothing new" and exits without clicking anything.
    """
    candidates, explicit = candidate_sources(source)
    tried = []
    skip = already_uploaded_filenames()
    saw_only_uploaded = False
    ...
    for candidate in candidates:
        ...
        video = newest_video_in(source_path, skip_names=skip)
        if video:
            log(f"Source: using folder {source_path}")
            return video

        # Nothing fresh here. Distinguish "folder is empty" from "everything in
        # it has already been uploaded" so the operator sees the real reason.
        total = _count_videos_in(source_path)
        if total > 0:
            saw_only_uploaded = True
            log(
                f"Source: {source_path} - {total} video(s) present but all recorded "
                f"as already uploaded, skipping"
            )
            if explicit:
                raise FileNotFoundError(
                    f"Every video in {source_path} is already recorded as uploaded "
                    f"in {UPLOAD_STATE_FILE}. Remove entries from that file (or pass "
                    f"the file path directly) to force a re-upload."
                )
        else:
    ...
    if saw_only_uploaded:
        # Explicit case has already raised above; here we know it's auto mode
        # and every candidate we could reach only held already-uploaded files.
        return None
...
def main():
    ...
    if video_path is None:
        ...
        log("Every video in the source folder is already recorded in:")
        log(f"  {UPLOAD_STATE_FILE}")
        log("Drop a new render in the toupload folder, or remove the file's entry")
        log("from uploaded.json (or pass its path explicitly) to re-upload it.")
        log("=" * 60)
        return 2
    log(f"Selected video: {video_path}")

    if args.check_only:
        log(f"--check-only: would upload {video_path.name}")
        return 0
    ...
    # Persistent record first: this is what actually protects against a
    # re-upload on the next run. It survives the move below failing, a crash
    # in the tidy-up path, and the toupload folder being restored later.
    try:
        mark_upload_recorded(video_path)
        log(f"Recorded upload in {UPLOAD_STATE_FILE}")
    except Exception as e:
        log(f"WARNING: could not update {UPLOAD_STATE_FILE}: {e}")
        log("Delete or edit the file entry before re-running or the same video")
        log("will be uploaded again.")

    # Tidy-up: move the file so the toupload folder shows only the queue. The
    # persistent record already protects against a re-upload, so a failed move
    # is a warning, not an error.
    log("Upload complete. Moving file out of the queue folder...")
    try:
