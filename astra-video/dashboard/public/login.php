...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sign in · Astra Video</title><meta name="theme-color" content="#0b0e14"><link rel="stylesheet" href="assets/app.css?v=4"></head><body class="auth-page"><main class="auth-card"><a class="brand" href="index.php"><span class="brand-mark">A</span><span class="brand-name">Astra<small>Video automation</small></span></a><h1>Sign in</h1><p class="muted">Manage channels, schedules and the publishing pipeline.</p><?php if($error): ?><p class="error" role="alert"><?=h($error)?></p><?php endif; ?><form method="post"><input type="hidden" name="csrf" value="<?=h($_SESSION['csrf'])?>"><label>Email address<input name="email" type="email" autocomplete="username" required autofocus></label><label>Password<input name="password" type="password" autocomplete="current-password" required></label><button class="button primary" type="submit">Sign in</button></form><p class="auth-foot"><a class="text-link" href="index.php">View the read-only dashboard</a></p></main></body></html>
