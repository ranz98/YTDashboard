...
```
YTCICDMASTER\
├── batch\        entry points (RUNPIPELINE, 1-DOWNLOAD, 2-EDIT, run, setup, uploader)
├── scripts\      Python (download, cover_caption, caption_text, edit_pending, trainer, upload_queue)
├── extension\    the Chrome extension - point "Load unpacked" here
│   └── icons\
├── assets\       fonts\ and buttons\ used by the pipeline
├── config\       deepseek-key.txt, find-extension-id.ps1
├── output\       runtime output - videos, logs, state files (created on first run)
└── README.md
```

Everything is driven from a `.bat` in `batch\`. The Python scripts and assets are internal — you should not have to `cd` into `scripts\` yourself.
...
```bash
batch\RUNPIPELINE.bat
```

Finds the newest Short, downloads it, then detects / reads / rewrites / redraws
the caption. `batch\RUNPIPELINE.bat @SomeHandle` targets another channel, and
`batch\RUNPIPELINE.bat <video-url>` skips the search and uses that video.
...
```bash
batch\RUNPIPELINE.bat @Clips_Edge cinematic
```
...
```bash
python scripts\cover_caption.py --rewrite --filter cinematic --preview 3
```
...
3. Select the `extension\` folder (the batch prints the full path)
...
Double-click **`batch\run.bat`**.

```
batch\run.bat                 # @Clips_Edge
batch\run.bat @SomeChannel    # any other channel
```

What happens:

1. `batch\run.bat` looks up the extension's ID and opens its trigger page in Chrome
...
5. `batch\run.bat` reads them, prints the outcome, and copies the URL to your clipboard
...
| `output\error.txt` | Only on failure — why it failed. `batch\run.bat` prints it and stops waiting |

## Downloading the video

`scripts\download.py` fetches the video and saves its first frame. With no arguments it
uses whatever `batch\run.bat` just captured:
...
| Command | Does |
| --- | --- |
| `python scripts\download.py` | the last captured Short |
| `python scripts\download.py <url> <url>` | specific videos |
| `python scripts\download.py --from-log` | every URL in `shorts-log.txt` |
| `python scripts\download.py --max-height 1080` | cap the resolution |
| `python scripts\download.py --frame-format png` | png frame instead of jpg |
| `python scripts\download.py --audio-only` | mp3, no frame |
| `python scripts\download.py --no-archive` | re-download something already fetched |
...
only-what's-new idea `batch\run.bat` uses. If a download turns out corrupt, its ID is
...
`CERTIFICATE_VERIFY_FAILED` even though normal browsing works. `scripts\download.py`
...
`scripts\cover_caption.py` finds the white caption box in `frame.jpg` and paints over it
for the whole video.

```bash
python scripts\download.py
python scripts\cover_caption.py
```

| Command | Does |
| --- | --- |
| `python scripts\cover_caption.py --rewrite` | rewrite with DeepSeek, draw it in the box |
| `python scripts\cover_caption.py` | newest download, cover the caption |
| `python scripts\cover_caption.py --detect-only` | find it and preview, render nothing |
| `python scripts\cover_caption.py --mode blur` | blur the region instead |
| `python scripts\cover_caption.py --mode pixelate` | pixelate it |
| `python scripts\cover_caption.py --color white` | fill with another colour |
| `python scripts\cover_caption.py --box 254,1203,1652,355` | skip detection, use these coords |
| `python scripts\cover_caption.py --no-ocr` | skip reading the caption text |
...
```
batch\run.bat ──reads the extension ID from the profile
              └─launches Chrome──▶ chrome-extension://<id>/trigger.html?channel=@Handle
                           ...
                           navigates that tab to the Short ──▶ plays
                                  ...
                                  writes output\*.txt
                                              │
batch\run.bat ◀──polls for the file──────────┘  then copies to clipboard
```
...
mirrors it live in the Chrome window while `batch\run.bat` prints a heartbeat.
...
| `extension\background.js` | Service worker: scraping, retries, file writing |
| `extension\trigger.html` / `trigger.js` | Page `batch\run.bat` opens; starts the grab, shows the live log |
| `extension\popup.html` / `popup.js` | Toolbar UI for manual runs, and the clipboard write |
| `extension\offscreen.html` / `offscreen.js` | Fallback blob-URL source for downloads |
| `config\find-extension-id.ps1` | Resolves the unpacked extension's ID from the profile |
| `config\deepseek-key.txt` | DeepSeek API key (also read from `--api-key` or `DEEPSEEK_API_KEY`) |
| `scripts\download.py` | Downloads the video and saves its first frame |
| `scripts\cover_caption.py` | Detects the white caption box and covers it |
| `scripts\edit_pending.py` | Walks the download tree and edits everything unedited |
| `batch\setup.bat` | One-time profile creation |
| `batch\run.bat` | Everyday entry point |
...
on the extension card, or just close that Chrome window and run `batch\run.bat` again.

## Troubleshooting

**`batch\run.bat` prints an error.** That text came from `output\error.txt`, written by
...
**`batch\run.bat` times out with nothing.** Look at the Chrome window it opened.
...
- Downloads always go to the profile's download folder — `batch\setup.bat` sets that
...
- `chrome-profile\` and `output\` are generated. Delete `chrome-profile\` to
  start over (you'd re-run `batch\setup.bat`).
